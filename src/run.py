...
if args.function == 'pretrain':
    ...
    #load model args if not none
    if args.reading_params_path is not None:
        model.load_state_dict(torch.load(args.reading_params_path))

    corruption_dataset = dataset.CharCorruptionDataset(open('wiki.txt', encoding='utf-8').read(), 128)
    name_dataset = dataset.NameDataset(corruption_dataset,
            open('birth_places_train.tsv', encoding='utf-8').read())
    tconf = trainer.TrainerConfig(max_epochs=75, batch_size=256, learning_rate=args.finetune_lr,
                        lr_decay=True, warmup_tokens=512*20, final_tokens=200*len(pretrain_dataset)*block_size,
                        num_workers=4, writer=writer)
    train_obj = trainer.Trainer(model, name_dataset, None, tconf)
    train_obj.train()
    # save parameters
    torch.save(model.state_dict(), args.writing_params_path)
